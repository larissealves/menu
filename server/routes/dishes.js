...
const SET_TIMEOUT = configs.timeout.api;
...
const requireAuth = (req, res, next) => {
  ...
  //=============== FILTERS FRONT-END ==========
  router.get('/get/dishes-id-relations/:filterOnlyActives', requireAuth, async (req, res) => {
    try {
  ...
  router.get('/get/imagesByDishId/:id', async (req, res) => {
    ...
    try {
      ...
      if (!images || images.length === 0) {
        res.status(404).json({ message: 'Nenhuma imagem encontrada para esse prato.' });
        return [];
      }
