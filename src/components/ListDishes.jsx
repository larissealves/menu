...
export default function ListAllDishes() {
  ...
  useEffect(() => {
   ...
   // Filtro local
  const filtered = listAllDishes.filter(dish => {
            ...
            {filtered.map((item) => (
              ...
              >
                  <div className="flex items-center flex-col md:flex-row justify-between gap-6">
                    <h3 className="flex-2 text-lg uppercase font-bold text-gray-800 break-all ">{item.name}</h3>
